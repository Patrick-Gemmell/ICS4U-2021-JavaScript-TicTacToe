...
function printBoard(gameList) {
  ...
  for (let listCounter = 0; listCounter < gameList.length; listCounter++) {
    ...
    for (let printCounter = 0;
      printCounter < gameList[listCounter].length; printCounter++) {
...
function moveScore(gameBoard, length, maxValue) {
  ...
  const score = winCondition(gameBoard, user, cpu);
  if (score == 10) {
    return score;
  } else if (score == -10) {
    return score;
    ...
    for (let rowCounter = 0; rowCounter < 3; rowCounter++) {
      for (let columnCounter = 0; columnCounter < 3; columnCounter++) {
        ...
        if (gameBoard[rowCounter][columnCounter] != user &&
          ...
          const tempValue = gameBoard[rowCounter][columnCounter];
          gameBoard[rowCounter][columnCounter] = cpu;
...
function computerMove(moveBoard) {
  ...
  for (let bestRow = 0; bestRow < 3; bestRow++) {
    for (let bestColumn = 0; bestColumn < 3; bestColumn++) {
      if (moveBoard[bestRow][bestColumn] != userSymbol &&
        ...
        const placeholdValue = moveBoard[bestRow][bestColumn];
        moveBoard[bestRow][bestColumn] = cpuSymbol;

        // Calculating the score of the computer's move
        const moveTotal = moveScore(moveBoard, 0, false);
  ...
  if (moveRow == 0 && moveColumn == 0) {
    ...
    throw new UserException('ERROR: Problem with determining computer move');
  }
}
...
try {
  ...
  while (gameCounter < 9) {
    ...
    console.log('');
    const userInput = prompt('Enter a move you would like to make (1 to 9): ');
    ...
    console.log('');
    const computerChoice = computerMove(userList);
    console.log('Computers Move: ' + parseInt(computerChoice, 10));
